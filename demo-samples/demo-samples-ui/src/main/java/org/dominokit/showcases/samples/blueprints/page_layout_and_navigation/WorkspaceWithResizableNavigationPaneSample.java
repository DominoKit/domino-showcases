...
public final class WorkspaceWithResizableNavigationPaneSample {
  ...
  public static PageLayoutSampleElement create() {
    ...
    DivElement layout = PageLayoutSupport.grid("240px minmax(0, 1fr)");
    DivElement pane = PageLayoutSupport.bordered("min-height: 250px;");
    DivElement content = PageLayoutSupport.bordered("min-height: 250px;");
    int[] paneWidth = {240};
    ...
    root.appendChild(PageLayoutSupport.header(
        ...
        .appendChild(PageLayoutSupport.row()
            .appendChild(PageLayoutSupport.action("Narrow", () -> { paneWidth[0] = Math.max(0, paneWidth[0] - 40); render.run(); }))
            .appendChild(PageLayoutSupport.action("Widen", () -> { paneWidth[0] = Math.min(360, paneWidth[0] + 40); render.run(); }))
